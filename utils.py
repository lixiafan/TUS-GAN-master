...
def load_csv_list(path):
    with open(path, 'r', encoding='utf-8') as f:
        f_str = f.readlines() # ['ID,CATE\n', '1.PNG,0\n', '10.PNG,0\n', '100.PNG,1\n',
        # print(f_str)
    head = f_str[0].split(',') # ['ID', 'CATE\n']
    if len(head) == 1:
        f_list = [l.split('\n')[0] for l in f_str[1:]]
        return f_list, []
    elif len(head) == 2:
        f_list = [l.split(',')[0] for l in f_str[1:]] # l : '1.PNG,0\n'
        label = [np.int(l.split(',')[1]) for l in f_str[1:]]
        return f_list, label

def crop_sub_imgs_fn(x, is_random=True):
    x = tl.prepro.flip_axis(x, axis=1, is_random=is_random)# 对图像进行左右翻转
    return tl.prepro.crop(x, wrg=224, hrg=224, is_random=is_random)# 裁剪图像

# np.load和np.save是读写磁盘数组数据的两个主要函数，默认情况下，数组是以未压缩的原始二进制格式保存在扩展名为.npy的文件中。
# savez函数输出的是一个压缩文件(扩展名为npz)，其中每个文件都是一个save函数保存的npy文件，文件名对应于数组名。
def load_and_assign_npz(sess, model_path, model_name, var_list):# 从磁盘上的二进制数组文件中读入数据
...
def eval_IoU(output, target, threshold=0.5, axis=(1, 2, 3), smooth=1e-5):
    ...
    无差别的联合交集（IoU）用于比较两批数据的相似性，通常用于评估二元图像分割。
    Parameters
    ----------
    ...
    smooth    : float            : This small value will be added to the numerator分子 and denominator分母
